...
class Mapping:
    def __init__(self):
        ...
        rospy.Subscriber('/carrot_team/is_still', Bool, self.sub_is_still)
        # rospy.Subscriber('/red/mavros/imu/data', Imu, self.get_imu)
        rospy.Subscriber('/red/imu', Imu, self.get_imu)
    ...
    def sub_is_still(self, msg):
        self.is_still = msg.data
...
if __name__ == "__main__" :
    ...
    while True:
        ...
        # Rotation
        drone_q = mp.drone_orientation
        # change nan to 10
        img_d_y = np.nan_to_num(mp.img_depth, nan = 10) * mp.mask
        # Rotate
        # rot_mat = R.from_quat(np.array([-1 * drone_q[0], -1 * drone_q[2], -1 * drone_q[1], drone_q[3]])).as_matrix()
        rot_mat = R.from_quat(drone_q).as_matrix()


        # change nan to 10 remainder
        ...
        # For rotation
        d_all = np.array([])
        d_all = np.append(d_all, img_d_x)
        d_all = np.append(d_all, -1 * img_d_y)
        d_all = np.append(d_all, -1 * img_d_z)
        d_all = np.reshape(d_all, (3, -1))

        d_all_rot = rot_mat.T @ d_all # rot_mat.T

        # print(np.shape(d_all_rot))

        # img_d_x_new, img_d_y_new, img_d_z_new = d_all_rot
        # img_d_y_new, img_d_x_new, img_d_z_new = d_all_rot
        # img_d_x_new = img_d_x_new * (-1)
        # img_d_z = img_d_z_new

        img_d_x_new, img_d_y_new, img_d_z_new = d_all_rot
        # img_d_x_new = img_d_x_new
        # img_d_y_new = -1 * img_d_y_new
        img_d_z = img_d_z_new
